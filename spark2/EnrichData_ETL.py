...
#---------------------------------------------------
print("JOB STARTED...")
car_sales     = spark.sql(f"SELECT * FROM {prefix}_CAR_DATA.car_sales")
customer_data = spark.sql(f"SELECT * FROM {prefix}_CAR_DATA.customer_data")
car_installs  = spark.sql(f"SELECT * FROM {prefix}_CAR_DATA.car_installs")
factory_data  = spark.sql(f"SELECT * FROM {prefix}_CAR_DATA.experimental_motors")
geo_data      = spark.sql(f"SELECT postalcode as zip, latitude, longitude FROM {prefix}_CAR_DATA.geo_data_xref")
...
salesandcustomers_sql = f"SELECT customers.*, sales.sale_date, sales.saleprice, sales.model, sales.VIN \
                            FROM {prefix}_CAR_DATA.car_sales sales JOIN {prefix}_CAR_DATA.customer_data customers \
...
#---------------------------------------------------
tempTable.write.mode("overwrite").saveAsTable(f'{prefix}_CAR_DATA.experimental_motors_enriched', format="parquet")
print(f"\tNEW ENRICHED TABLE CREATED: {prefix}_CAR_DATA.EXPERIMENTAL_MOTORS_ENRICHED")
tempTable.show(n=5)
